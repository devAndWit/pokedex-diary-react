...
export async function fetchAllPokemon() {
    ...
    return await Promise.all(pokemonSimpleDataList.map(async (pokemon) => {
...
export async function getAbilitiesWithDescription(abilities) {
    try {
        return await Promise.all(abilities.map(async (ability) => {
            const abilityData = await axios.get(ability.ability.url);

            const description = abilityData.data.effect_entries.find((entry) => entry.language.name === "en")?.effect;
...
export async function getTypesWithSymbol(types) {
    return types?.map((type) => ({name: type.type.name, symbol: typeSymbols[type.type.name] || null}));
}
